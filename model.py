...
def collect_model_parameters_into_list(encoder_layer_params, decoder_layer_params, embedding_params):
    ...
    def add_parameters(param_dict):
        for param in param_dict.values():
            param_id = id(param)
            if param_id not in seen_tensor_ids:
                flat_params_list.append(param)
                seen_tensor_ids.add(param_id)

    for layer_dict in encoder_layer_params:
        add_parameters(layer_dict)
...
def shift_targets_right_with_start_token(target_ids, start_token_id):
    ...
    shifted_targets = torch.cat([start_column, target_ids[:, :-1]], dim=1)
...
def compute_noam_learning_rate(step, d_model, warmup_steps):
    ...
    decay_phase = step ** -0.5
    warmup_phase = step * (warmup_steps ** -1.5)

    return (d_model ** -0.5) * min(decay_phase, warmup_phase)
...
def build_uniform_smoothing_distribution(shape, vocab_size, epsilon):
    ...
    smoothing_tensor = torch.full(shape, smoothing_value, dtype=torch.float)

    return smoothing_tensor
...
def set_confidence_on_gold_tokens(smoothed_distribution, gold_token_ids, confidence):
    ...
    gold_index = gold_token_ids.unsqueeze(-1)

    output.scatter_(-1, gold_index, confidence)
...
def zero_pad_column_and_pad_token_rows(smoothed_distribution, gold_token_ids, pad_id):
    ...
    pad_positions = (gold_token_ids == pad_id)
    output[pad_positions] = 0.0
...
def compute_label_smoothed_kl_loss(log_probabilities, smoothed_distribution):
    ...
    pointwise_loss = -smoothed_distribution * log_probabilities
...
def average_loss_over_non_pad_tokens(total_loss, gold_token_ids, pad_id):
    ...
    safe_count = non_pad_count.clamp(min=1)

    return total_loss / safe_count.to(total_loss.dtype)
...
def compute_token_accuracy_ignoring_pad(log_probabilities, gold_token_ids, pad_id):
    ...
    total_non_pad = non_pad_mask.sum()

    if total_non_pad.item() == 0:
        return torch.tensor(0.0)

    return correct.sum().float() / total_non_pad.float()
...
def update_adam_second_moment(v_prev, grad, beta2):
    ...
    with torch.no_grad():
        v_t = v_prev * beta2 + (1 - beta2) * grad ** 2
...
def apply_adam_bias_correction(m_t, v_t, beta1, beta2, step):
    ...
    return m_hat.detach(), v_hat.detach()
...
def compute_batch_training_loss(src_batch, tgt_batch, model_params, config):
    ...
    smoothed_dist = build_uniform_smoothing_distribution(log_probs.shape, vocab_size, smoothing)
...
def append_tokens_to_beam_sequences(beam_sequences, beam_indices, token_ids):
    ...
    # 1. Gather the parent sequences that correspond to the top-k candidates
    parent_sequences = beam_sequences[beam_indices]
    
    # 2. Reshape the new token ids into a column vector (k, 1)
    new_tokens_column = token_ids.unsqueeze(-1)
    
    # 3. Concatenate the new tokens to the end of the parent sequences
...
def mark_finished_beams(token_ids, finished_flags, end_token_id):
    ...
    # Check if the newly generated token is the end token
